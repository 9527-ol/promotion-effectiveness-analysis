...
-- select * from sales_2 limit 10 
-- ;
-- SELECT *, sum(r_count) OVER (PARTITION BY id ORDER BY id, year, week ROWS 2 PRECEDING) as test
-- from (
-- select * from sales_2 ORDER BY id, year, week limit 10 ) as k
-- ;
...
SELECT 
    ...
    , COUNT(*) FILTER (WHERE (promo_units + reg_units) > 900 ) OVER (PARTITION BY id ORDER BY id, year, week ROWS 7 PRECEDING) - (CASE WHEN (promo_units + reg_units) > 900 THEN 1 ELSE 0 END) as gt_900_week_count
...
(
	...
	UNION ALL  -- 1. pay attention to the difference between "union all" and "union".
						 -- 2. Use "union all" in this case is for showing, otherwise it could be done by only code block above.
	SELECT 
		 a.*
		 , null 
		 , null 
...
-- SELECT id, year, week, type, cast(cat_week_promo_pct as DECIMAL(10,3)), type_2 FROM sales_4 limit 100
-- ;
-- SELECT * FROM sales_4 WHERE category='A1' ORDER BY year, week and year=2017 and week=1
-- ;




-- Task 5: Data Calculation
...
select id, year ,week ,grp, id_row_num, lag_1_up_row_num, reg_units_1_up from sales_6 where id='0005'
;
select id, year, week, type_2, flag, b0, f0, window_size, window_frame, window_start_or_end from sales_6 where id='0005' and year=2018 limit 1000 --window_size >=3 
;



-- Task 7: Dada Calculation
...
-- select id, year, week , flag, f0, type_2, promo_units, reg_units, estimated_reg_units, window_size, window_frame
-- 	, window_frame_reg_units_up, window_frame_reg_units_down, grp_2, window_frame_reg_units_1, window_frame_reg_units_2, window_frame_estimated_units_1, window_frame_estimated_units_2
-- 	, up_degree, up_degree_2,  promo_weight, lifted_estimated_reg_units_raw, window_sum_lifted_estimated_units_raw, estimated_reg_units_2, window_start, window_end
-- from sales_7 
-- where id='0005'
-- order by id, year, week limit 1000
-- ;
-- 
-- select 
